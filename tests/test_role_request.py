...
def test_get_role_request(
    ...
    assert okta_group_role_request is None
    ...
    role_request_url = url_for("api-role-requests.role_request_by_id", role_request_id=role_group_role_request.id)
    rep = client.get(role_request_url)
...
#     data = rep.get_json()
#     access_request = db.session.get(RoleRequest, data["id"])
#     access_owner = OktaUser.query.filter(OktaUser.email == app.config["CURRENT_OKTA_USER_EMAIL"]).first()
...
#     access_requests = RoleRequestFactory.create_batch(10, requester_user_id=user.id, requested_group_id=okta_group.id)
...
#     access_request = CreateRoleRequest(
#         requester_user=user,
#         requested_group=okta_group,
#         request_ownership=False,
#         request_reason="test reason",
...
#     ApproveRoleRequest(access_request=access_request, approver_user=access_owner).execute()
...
#     RejectRoleRequest(access_request=access_request, current_user_id=access_owner).execute()
