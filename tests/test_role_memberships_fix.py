...
def test_missing_user_from_group_membership(db: SQLAlchemy, mocker: MockerFixture, role_group: RoleGroup, okta_group: OktaGroup, user: OktaUser) -> None:
    db.session.add(user)
    ...
    assert (
        OktaUserGroupMember.query.filter(
            OktaUserGroupMember.role_group_map_id == member_role_group_map.id
        )
    ...
    assert (
        OktaUserGroupMember.query.filter(
            OktaUserGroupMember.role_group_map_id == owner_role_group_map.id
        )
        .filter(OktaUserGroupMember.ended_at.is_(None))
        .count()
        == 1
...
def test_missing_user_from_group_membership_with_expiring_role_membership(db: SQLAlchemy, mocker: MockerFixture, role_group: RoleGroup, okta_group: OktaGroup, user: OktaUser) -> None:
    db.session.add(user)
    db.session.add(role_group)
    db.session.add(okta_group)
    db.session.add(OktaUserGroupMember(user_id=user.id, group_id=role_group.id, ended_at=datetime.now(UTC) + timedelta(days=2)))
    member_role_group_map = RoleGroupMap(
        role_group_id=role_group.id, group_id=okta_group.id, is_owner=False
    )
    db.session.add(member_role_group_map)
...
def test_missing_user_from_group_membership_with_expiring_role_assignment(db: SQLAlchemy, mocker: MockerFixture, role_group: RoleGroup, okta_group: OktaGroup, user: OktaUser) -> None:
    db.session.add(user)
    db.session.add(role_group)
    db.session.add(okta_group)
    db.session.add(OktaUserGroupMember(user_id=user.id, group_id=role_group.id, ended_at=datetime.now(UTC) + timedelta(days=4)))
    member_role_group_map = RoleGroupMap(
        role_group_id=role_group.id, group_id=okta_group.id, is_owner=False, ended_at=datetime.now(UTC) + timedelta(days=2)
    )
    db.session.add(member_role_group_map)
    owner_role_group_map = RoleGroupMap(
        role_group_id=role_group.id, group_id=okta_group.id, is_owner=True, ended_at=datetime.now(UTC) + timedelta(days=6)
    )
    db.session.add(owner_role_group_map)
    db.session.commit()
...
def test_missing_user_from_group_membership_with_both_expiring(db: SQLAlchemy, mocker: MockerFixture, role_group: RoleGroup, okta_group: OktaGroup, user: OktaUser) -> None:
    db.session.add(user)
    db.session.add(role_group)
    db.session.add(okta_group)
    db.session.add(OktaUserGroupMember(user_id=user.id, group_id=role_group.id))
    ...
    db.session.add(member_role_group_map)
    owner_role_group_map = RoleGroupMap(
        role_group_id=role_group.id, group_id=okta_group.id, is_owner=True, ended_at=datetime.now(UTC) + timedelta(days=4)
    )
    db.session.add(owner_role_group_map)
    db.session.commit()
...
def test_extra_user_from_role_membership(db: SQLAlchemy, mocker: MockerFixture, role_group: RoleGroup, okta_group: OktaGroup, user: OktaUser) -> None:
    db.session.add(user)
    db.session.add(role_group)
    db.session.add(okta_group)
