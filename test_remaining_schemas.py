...
with flask_app.app_context():
    try:
        ...
        from api_v2.schemas.role_memberships import RoleMemberUpdate, RoleMemberRead
        ...
        role_read = RoleMemberRead(
            groups_in_role=valid_group_ids,
            groups_owned_by_role=[valid_group_ids[0]]
        )
        ...
        metrics_data_duration = MetricsData(
        ...
        metrics_data_histogram = MetricsData(
            name="response_times",
            value=1.5,
        ...
        counter_metric = Metrics(
            type=MetricType.COUNTER,
            data=metrics_data_value
        )
        print(f"✓ Counter metric: {counter_metric.type}")
        
        timing_metric = Metrics(
            type=MetricType.TIMING,
            data=metrics_data_duration
        )
        print(f"✓ Timing metric: {timing_metric.type}")
        
        histogram_metric = Metrics(
            type=MetricType.HISTOGRAM,
            data=metrics_data_histogram
        )
        ...
        print(f"✓ JSON serialization works for all schemas")
