"""
Check for self-add constraint for FastAPI.
...
class CheckForSelfAdd:
    ...
    def execute_for_group(self) -> Tuple[bool, str]:
        if self.current_user is None or AuthorizationHelpers.is_access_admin(self.db, self.current_user):
            return True, ""
        ...
        if (
            ...
            .filter(
                or_(
                    ...
                    OktaUserGroupMember.ended_at > self.db.func.now(),
                )
            )
            .count()
            > 0
