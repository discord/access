...
from api_v2.schemas import AuditEventType, AuditLogRead, AuditGroupSummary, AuditAppSummary, AuditUserSummary, AuditUserGroupMembershipSummary
...
class ModifyGroupUsers:
    def __init__(
        ...
        *,
        ...
        if len(members_to_add) > 0:
            self.members_to_add = (
                self.db.query(OktaUser).filter(OktaUser.id.in_(members_to_add)).filter(OktaUser.deleted_at.is_(None)).all()
            )

        self.owners_to_add = []
        if len(owners_to_add) > 0:
            self.owners_to_add = (
                self.db.query(OktaUser).filter(OktaUser.id.in_(owners_to_add)).filter(OktaUser.deleted_at.is_(None)).all()
            )

        self.members_should_expire = []
        if len(members_should_expire) > 0:
            self.members_should_expire = (
                self.db.query(OktaUserGroupMember).filter(OktaUserGroupMember.id.in_(members_should_expire))
        ...
        if len(owners_should_expire) > 0:
            self.owners_should_expire = (
                self.db.query(OktaUserGroupMember).filter(OktaUserGroupMember.id.in_(owners_should_expire))
                .filter(OktaUserGroupMember.ended_at > func.now())
        ...
        if len(members_to_remove) > 0:
            self.members_to_remove = (
                self.db.query(OktaUser).filter(OktaUser.id.in_(members_to_remove)).filter(OktaUser.deleted_at.is_(None)).all()
            )

        self.owners_to_remove = []
        if len(owners_to_remove) > 0:
            self.owners_to_remove = (
                self.db.query(OktaUser).filter(OktaUser.id.in_(owners_to_remove)).filter(OktaUser.deleted_at.is_(None)).all()
            )
    ...
    def _log_audit_event(self) -> None:
        """Log audit event for group user modifications."""
        email = None
        ...
        audit_data = {
            ...
            "owners_removed_ids_emails": [
                ...
                ) for user in self.owners_to_remove
            ],
            ...
            "members_removed_ids_emails": [
                AuditUserSummary(
                    ...
                    last_name=user.last_name,
                    display_name=user.display_name
                ) for user in self.members_to_remove
            ],
            ...
            "members_should_expire_user_id_group_id": [
                AuditUserGroupMembershipSummary(
                    user_id=membership.user_id,
                    group_id=membership.group_id
                ) for membership in self.members_should_expire
            ],
        }
    ...
    async def _execute(self) -> OktaGroup:
        ...
        if len(self.members_to_remove) > 0 or len(self.owners_to_remove) > 0:
            ...
            removed_users_with_other_access = (
                self.db.query(OktaUserGroupMember).with_entities(OktaUserGroupMember.user_id, OktaUserGroupMember.is_owner)
                .filter(
                    or_(
                        OktaUserGroupMember.ended_at.is_(None),
                        OktaUserGroupMember.ended_at > func.now(),
            ...
            if type(self.group) is RoleGroup and self.sync_to_okta:
                role_associated_groups_mappings = (
                    self.db.query(RoleGroupMap).options(joinedload(RoleGroupMap.active_group))
                ...
                removed_role_group_users_with_other_access = (
                    self.db.query(OktaUserGroupMember).with_entities(
        ...
        if len(self.members_to_add) > 0 or len(self.owners_to_add) > 0:
            ...
            pending_requests_query = (
                self.db.query(AccessRequest).options(joinedload(AccessRequest.requested_group))
