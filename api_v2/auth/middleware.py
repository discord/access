...
from api_v2.database import SessionLocal
from api_v2.models import OktaUser
from api_v2.auth.dependencies import get_cloudflare_auth, get_oidc_auth
...
class AuthenticationMiddleware(BaseHTTPMiddleware):
    ...
    def _is_public_route(self, request: Request) -> bool:
        ...
        for route in request.app.routes:
            ...
            if match == Match.FULL:
                # Check if the endpoint has the _public attribute set by mark_as_public decorator
    ...
    async def _authenticate_user(self, request: Request) -> Optional[OktaUser]:
        """
        ...
        try:
            ...
            # Cloudflare Access authentication
                ...
                try:
                    ...
                    payload = await cloudflare_auth.verify_cloudflare_token(request)
            ...
            # OIDC authentication
                ...
                try:
                    ...
                    payload = await oidc_auth.verify_oidc_token(request)
...
def get_authenticated_user(request: Request) -> OktaUser:
    """
    Get the authenticated user from the request state.
    This should be used in endpoints that need access to the current user.
    
    Args:
        request: The FastAPI request object
        
    Returns:
        The authenticated OktaUser
        
    Raises:
        AttributeError: If the user is not in the request state (middleware not run)
    """
