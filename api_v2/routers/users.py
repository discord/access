...
from api_v2.schemas.users import UserDetail, UserList
...
async def list_users(
    ...
    _: OktaUser = Depends(get_current_user),  # Ensure user is authenticated
    db: Session = Depends(get_db)
):
    """
    ...
    query = (
        ...
        .order_by(func.lower(OktaUser.email))
    )
...
async def get_user_audit(user_id: str):
    ...
    raise HTTPException(
        status_code=501, 
        detail="User audit endpoint not yet implemented in FastAPI version"
    )