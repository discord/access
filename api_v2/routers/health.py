...
from api_v2.auth import get_current_user_optional, public_route
...
async def health_check(db: Session = Depends(get_db)) -> Dict[str, Any]:
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail={"status": "error", "error": str(e)}
        )


@router.get("/healthz/auth")
@public_route
async def health_check_auth(current_user: OktaUser = Depends(get_current_user_optional)) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        return {"status": "healthy", "version": "2.0.0", "authentication": "not_authenticated"}
