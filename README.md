...
- **App-group-lifecycle hooks run *after* the HTTP response, not during it.** Access commits the change, answers the request, and then reconciles, so your hook may take as long as the external system needs without holding a user waiting. Two consequences worth designing for: the group Access hands you is re-loaded at that point, so it reflects committed state rather than a snapshot (and a group deleted in between is skipped, not reconciled); and whatever the hook writes is absent from the create/update response the user just got. Declare `pending_values` on the status property that carries your reconcile state (e.g. `pending_values=("pending",)` on a `sync_status`) and the UI keeps refreshing the group until it moves off them. Outside a request — the `sync-app-groups` CronJob, other CLI commands — hooks still run inline.
