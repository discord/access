...
- **Declare hook implementations with `async def`.** This applies to the notification hooks, the conditional-access hooks, the metrics `record_*` / `set_global_tags` / `flush` hooks, and the app-group-lifecycle *lifecycle* hooks (`group_created`, `group_updated`, `group_deleted`, `group_members_added`, `group_members_removed`, `sync_group`). Access fails fast at startup with a clear error if one of these is registered as a plain `def`.
...
- **Plugins integrate with Access only through the plugin interface.** A plugin is self-contained: it never imports `api.operations` or `api.services`, never builds a query, and never touches a database session. `api.models` is fine — the hookspecs are written in terms of those types. If your plugin needs something Access can do, open an issue so it can be added to the interface as a general capability rather than reaching past it.
- **Access owns the transaction: never commit or roll back in a hook.** Access commits after your hook returns and rolls back if it raises. Because that rollback expires every ORM instance Access holds, mutate state only through what the hook was given, and don't hold ORM objects across hook calls.
- **App-group-lifecycle hooks receive an `AppGroupLifecycleContext` rather than a session.** It is bound to your plugin's id — so you never pass `plugin_id` to a capability call, and can't touch another plugin's `plugin_data` — and it carries every Access interaction the hook is allowed. Its method docstrings are the contract, including which status writes survive a failed hook and which deliberately do not. [app_group_lifecycle_google](https://github.com/discord/access/tree/main/examples/plugins/app_group_lifecycle_google) is a full reference implementation.
- **App-group-lifecycle hooks run *after* the HTTP response, not during it.** Access commits the change, answers the request, and then reconciles, so your hook may take as long as the external system needs without holding a user waiting. Outside a request — the `sync-app-groups` CronJob, other CLI commands — hooks still run inline.
- **So write those hooks as reconciliations of current state, not as applications of the delta they are handed.** The group is re-loaded when the hook runs, so it reflects committed state rather than a snapshot (and a group deleted in between is skipped, not reconciled), but the delta arguments are not re-derived: `old_name`/`old_description` are snapshots that may no longer bracket a single rename, and fires from *different* requests are not ordered relative to each other, so "added" and "removed" for the same user can arrive in either order. Take `ctx.lock` if you need mutual exclusion — it is the only mechanism that holds across workers. Treat `members` and the before/after values as a hint about what changed.
- **Delivery is not guaranteed, and `sync_group` is only as much of a safety net as you make it.** A worker killed between the response and the drain drops the fire, as does a request that fails after queuing one. Access does not police what `sync_group` does, so a plugin that implements it as a full idempotent reconciliation converges afterwards and one that does not, does not. One gap it cannot close either way: it sees only current membership, so a delta-based revoke cannot recover a lost `group_members_removed`. If your plugin records events rather than converging state — an audit or notification plugin — expect gaps.
- **`group_deleted` is the exception: it runs inline, and it retries.** The sweep skips soft-deleted groups, so nothing would revisit a delete that went missing and the external group would outlive the Access one. So your `group_deleted` blocks the API response — keep it as quick as the external system allows — and if it raises, Access re-fires it on each `sync-app-groups` run until it succeeds or the retry budget runs out. **Make it idempotent:** deleting an external group that is already gone is a success, not an error.
- **Whatever a hook writes is absent from the create/update response the user just got.** Declare `pending_values` on the status property that carries your reconcile state (e.g. `pending_values=("pending",)` on a `sync_status`) and the UI keeps refreshing the group until it moves off them. If you declare them, write a status on *every* reconcile outcome, including the ones where you decide there is nothing to do — an empty status reads as "the hook has not run yet" and the UI will poll for it.
- **Request-path notification hooks receive read-only, detached ORM snapshots.** Every notification raised from an HTTP request — the `*_created` and `*_completed` hooks for access, role, and group requests — runs in a post-response background task, after the request's DB session has been torn down, so Access detaches (`expunge`s) the objects it passes. Read only the attributes Access already loaded — the request, its group/role, a group request's `requested_app`, the requester, and the approvers, and their columns — and don't trigger a lazy load / re-query (there's no live session behind them) or expect mutations to persist. If your notifier needs a wider object graph than what's passed, open an issue so the operation can eager-load it. (The `access_expiring_*` hooks are the exception — they run inline in the syncer CronJob.)
