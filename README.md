...
- **Plugins integrate with Access only through the plugin interface.** A plugin is self-contained: it never imports `api.operations` or `api.services`, never builds a query, and never touches a database session. If your plugin needs something Access can do, that belongs on the interface as a capability — open an issue rather than importing past it. `api.models` is fine to import for the types the hookspecs are written in terms of.
- **App-group-lifecycle hooks receive an `AppGroupLifecycleContext`, not a session.** It is bound to your plugin's id, so you never pass `plugin_id` to a capability call and cannot touch another plugin's `plugin_data`. It provides `lock`, `find_groups_by_status`, `get_config`/`set_config`/`get_status`/`set_status`, `set_group_description`, and the Okta group-push calls. See [app_group_lifecycle_google](https://github.com/discord/access/tree/main/examples/plugins/app_group_lifecycle_google) for a full reference implementation.
- **Access owns the transaction: never commit or roll back in a hook.** Access commits after your hook returns, and rolls back if it raises. Because a rollback expires every ORM instance Access is holding, mutate state only through the context and do not hold ORM objects across hook calls.
- **`set_status(..., durable_on_failure=True)` for diagnostics that must survive your own failure** — a sync status, an error string. Access re-applies those in a fresh transaction after a failed hook, so an operator can still see why reconciliation failed. Leave the flag off for anything that records ownership or identity of an external resource (an external group id, a mapping id): those are only sound when committed in the same transaction as the `lock`-protected check that justified them, and replaying one afterwards could let two Access groups claim the same external resource.
- **`sync_group` reconciles one group per call.** Access invokes it once per app group, each in its own transaction, so don't write a batch loop or swallow failures — let exceptions propagate and Access will isolate that group and report the run as failed.
