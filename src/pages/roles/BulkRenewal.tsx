...
function BulkRenewalDialog(props: BulkRenewalDialogProps) {
  ...
  const {data: rowConstraints} = useConstraintsForGroups(allRowGroupIds);

  const cantAddSelf = (group: GroupRefForMembership | null | undefined, owner: boolean) =>
    effectiveOwnerCantAddSelf(group?.id ? rowConstraints?.by_group?.[group.id] : undefined, owner);
  ...
  const ownerTimeLimit = effectiveTimeLimit(ownerSideConstraints?.coalesced, true);
  const memberTimeLimit = effectiveTimeLimit(memberSideConstraints?.coalesced, false);
  ...
  const requiredReason =
    effectiveRequiredReason(ownerSideConstraints?.coalesced, true) ||
    effectiveRequiredReason(memberSideConstraints?.coalesced, false);
