...
class NotificationPluginSpec:
    ...
    **The ORM instances passed to the request-path hooks are read-only, detached
    snapshots.** Every request-path notification — the `*_created` and
    ...
    the values as a snapshot taken when the operation ran. If a hook needs a wider
