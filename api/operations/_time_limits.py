...
records it derives from, the user's membership in the role and the role's
association with the group. `ModifyRoleGroups` and `ModifyGroupUsers`
establish that when they create such a row, by taking the minimum of the two.
Anything that later shortens one of those inputs has to re-establish it.
"""
