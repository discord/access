...
class ModifyGroupUsers:
    def __init__(
        ...
        *,
        ...
        if len(members_to_add) > 0:
            self.members_to_add = (
                OktaUser.query.filter(OktaUser.id.in_(members_to_add))
        ...
        if len(owners_to_add) > 0:
            self.owners_to_add = (
                OktaUser.query.filter(OktaUser.id.in_(owners_to_add))
                .filter(OktaUser.deleted_at.is_(None))
                .all()
            )
        ...
        if len(members_to_remove) > 0:
            self.members_to_remove = (
                OktaUser.query.filter(OktaUser.id.in_(members_to_remove))
                .filter(OktaUser.deleted_at.is_(None))
                .all()
            )
        ...
        if len(owners_to_remove) > 0:
            self.owners_to_remove = (
                OktaUser.query.filter(OktaUser.id.in_(owners_to_remove))
                .filter(OktaUser.deleted_at.is_(None))
                .all()
            )
    ...
    async def _execute(self) -> OktaGroup:
        ...
        if (len(self.members_to_add) +
           len(self.members_to_remove) +
           len(self.owners_to_add) +
           len(self.owners_to_remove) == 0):
            return self.group
        ...
        if len(remove_changed_members) > 0 or len(remove_changed_owners) > 0:
            OktaUserGroupMember.query.filter(
                ...
                OktaUserGroupMember.user_id.in_([m.id for m in remove_changed_members])
            ).filter(
            ...
            OktaUserGroupMember.query.filter(
                ...
                OktaUserGroupMember.user_id.in_([m.id for m in remove_changed_owners])
            ).filter(
                OktaUserGroupMember.role_group_map_id.is_(None)
            ).update(
                {OktaUserGroupMember.ended_at: db.func.now(),
                ...
                role_associated_group_memberships = (
                    ...
                    .filter(
                        OktaUserGroupMember.user_id.in_(
                            [m.id for m in remove_changed_members]
                        )
                    )
                    .filter(
                        OktaUserGroupMember.role_group_map_id.in_(
                            [m.id for m in role_associated_groups_mappings]
                        )
                    )
                )
        ...
        if len(self.members_to_remove) > 0 or len(self.owners_to_remove) > 0:
            ...
            removed_users_with_other_access = (
                OktaUserGroupMember.query.with_entities(
                    OktaUserGroupMember.user_id, OktaUserGroupMember.is_owner
                )
                .filter(
                    db.or_(
                        OktaUserGroupMember.ended_at.is_(None),
                ...
                .filter(
                    OktaUserGroupMember.user_id.in_(
                        members_to_remove_ids + owners_to_remove_ids
                    )
                )
            ...
            if self.sync_to_okta and self.group.is_managed:
                for member_id in okta_members_to_remove_ids:
                    ...
                    async_tasks.append(
                        asyncio.create_task(
                            okta.async_remove_user_from_group(self.group.id, member_id)
                        )
                    )

            removed_owners_with_other_access_ids = [
                m.user_id for m in removed_users_with_other_access if m.is_owner
            ]
            ...
            if self.sync_to_okta and self.group.is_managed:
                for owner_id in okta_owners_to_remove_ids:
                    ...
                    async_tasks.append(
                        asyncio.create_task(
                            okta.async_remove_owner_from_group(self.group.id, owner_id)
                        )
                    )
        ...
        if len(self.members_to_add) > 0 or len(self.owners_to_add) > 0:
            ...
            for member in self.members_to_add:
                if self.sync_to_okta and self.group.is_managed:
                    ...
                    async_tasks.append(
                        asyncio.create_task(
                            okta.async_add_user_to_group(self.group.id, member.id)
                        )
                    )
            ...
            for owner in self.owners_to_add:
                if self.sync_to_okta and self.group.is_managed:
                    ...
                    async_tasks.append(
                        asyncio.create_task(
                            okta.async_add_owner_to_group(self.group.id, owner.id)
                        )
                    )
            ...
            if type(self.group) == RoleGroup:
                ...
                for role_associated_group_map in role_associated_groups_mappings:
                    ...
                    if role_associated_group_map.is_owner:
                        group_ownerships_added[
                            role_associated_group_map.group_id
                        ] = role_associated_access_added
                    else:
                        group_memberships_added[
                            role_associated_group_map.group_id
                        ] = role_associated_access_added
            ...
            pending_member_requests = (
                pending_requests_query.filter(
                    AccessRequest.requested_group_id.in_(group_memberships_added.keys())
                )
                .filter(
                    AccessRequest.requester_user_id.in_([m.id for m in self.members_to_add])
                )
                .filter(AccessRequest.request_ownership.is_(False))
                .all()
            )
            for access_request in pending_member_requests:
                async_tasks.append(self._approve_access_request(
                    access_request,
            ...
            pending_owner_requests = (
                ...
                .filter(
                    AccessRequest.requester_user_id.in_([m.id for m in self.owners_to_add])
                )
    ...
    def _approve_access_request(
            self, access_request: AccessRequest, added_okta_user_group_member: OktaUserGroupMember) -> asyncio.Task[None]:
