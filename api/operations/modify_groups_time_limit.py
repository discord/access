...
class ModifyGroupsTimeLimit:
    def __init__(
        ...
        self.tags = (
            Tag.query.filter(Tag.id.in_(tags))
    ...
    def execute(self) -> None:
        ...
        membership_seconds_limit = coalesce_constraints(
            Tag.MEMBER_TIME_LIMIT_CONSTRAINT_KEY,
            self.tags
        )
        ownership_seconds_limit = coalesce_constraints(
        ...
        if membership_seconds_limit is not None:
            ...
            RoleGroupMap.query.filter(
                RoleGroupMap.group_id.in_([g.id for g in self.groups])
            ).filter(
            ...
            role_group_map_associations = RoleGroupMap.query.filter(
                RoleGroupMap.role_group_id.in_([g.id for g in self.role_groups])
            ).filter(
                RoleGroupMap.is_owner.is_(False)
            ).filter(
                db.or_(
