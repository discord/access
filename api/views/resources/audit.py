...
class UserGroupAuditResource(MethodResource):
    @FlaskApiSpecDecorators.request_schema(
        SearchUserGroupAuditPaginationRequestSchema, location="query"
    )
    ...
    def get(self) -> ResponseReturnValue:
        ...
        query = (
            ...
            .options(
                ...
                selectinload(OktaUserGroupMember.group)
                .options(
                ...
                selectinload(
                    OktaUserGroupMember.role_group_mapping
            ...
            query = (
                ...
                .order_by(
                    nulls_order(
                        getattr(
                            group_alias.name
                            if order_by == "moniker" else getattr(OktaUserGroupMember, order_by),
                            "desc" if order_direction else "asc"
                        )()
                    ),
                    group_alias.name.asc()
                    if order_by != "moniker" else nullslast(OktaUserGroupMember.created_at.asc())
                )
            )
        ...
        if "owner_id" in search_args:
            ...
            owner = (
                OktaUser.query
                .filter(db.or_(OktaUser.id == owner_id, OktaUser.email.ilike(owner_id)))
                .order_by(nullsfirst(OktaUser.deleted_at.desc()))
                .first_or_404()
            )
            ...
            app_owner_group_ownerships = (
                owner_group_ownerships.options(
                    joinedload(
                        OktaUserGroupMember.group.of_type(AppGroup)
                    )
                )
            ...
            app_groups_owned = (
                AppGroup.query.filter(AppGroup.app_id.in_([o.group.app_id for o in app_owner_group_ownerships.all()]))
            )
            ...
            app_groups_directly_owned_by_others_ids = set(
                g.group_id for g in
                app_groups_directly_owned_by_others.with_entities(OktaUserGroupMember.group_id).all()
            )
                ...
                query.filter(
                    db.or_(
                        OktaUserGroupMember.group_id.in_(
                            [o.group_id for o in owner_group_ownerships
                             .with_entities(OktaUserGroupMember.group_id).all()]
                        ),
                        OktaUserGroupMember.group_id.in_(
            ...
            if "group_id" in search_args:
                query = query.filter(
                    db.or_(
                        ...
                        (OktaUser.first_name + " " + OktaUser.last_name).ilike(
                            like_search
                        ),
                    )
                )
        ...
        if "direct" in search_args:
            if search_args["direct"]:
                query = (
                    query
                    .filter(db.not_(OktaUserGroupMember.active_role_group_mapping.has()))
                )
            ...
            query = query.filter(
                db.and_(
                    ...
                    OktaUserGroupMember.ended_at >
                        datetime.fromtimestamp(search_args["start_date"]),
                    OktaUserGroupMember.ended_at <
                        datetime.fromtimestamp(search_args["end_date"]),
                )
            )
...
class GroupRoleAuditResource(MethodResource):
    @FlaskApiSpecDecorators.request_schema(
        SearchGroupRoleAuditPaginationRequestSchema, location="query"
    )
    ...
    def get(self) -> ResponseReturnValue:
        ...
        query = (
            ...
            .options(
                joinedload(
                    RoleGroupMap.group.of_type(ROLE_ASSOCIATED_GROUP_TYPES)
                ).options(
                    joinedload(ROLE_ASSOCIATED_GROUP_TYPES.AppGroup.app),
        ...
        if "group_id" in search_args:
            ...
            query = (
                ...
                .order_by(
                    ...
                    RoleGroup.name.asc()
                    if order_by != "moniker" else nullslast(RoleGroupMap.created_at.asc())
                )
            )
