...
class RoleMemberSchema(Schema):
    ...
    owner_groups_to_remove = fields.List(fields.String(validate=validate.Length(equal=20)), required=True, load_only=True)
    ...
    def must_be_in_the_future(data: Optional[datetime]) -> None:
        if data and data < datetime.now():
            raise ValidationError(
                "Ended at datetime for add users must be in the future"
            )
    ...
    def convert_to_utc(self, item: Dict[str, Any], many: bool, **kwargs: Any) -> Dict[str, Any]:
        ...
        if "groups_added_ending_at" in item:
            item["groups_added_ending_at"] = item["groups_added_ending_at"].astimezone(
                tz=timezone.utc
            )
        return item
