...
class CreateGroupRequestSchema(Schema):
    ...
    requested_group_description = fields.String(
        validate=validate.Length(max=1024), required=False, load_only=True, load_default=""
    )
    requested_group_type = fields.String(
        required=True, load_only=True, validate=validate.OneOf(["okta_group", "role_group", "app_group"])
    )
    requested_app_id = fields.String(
        validate=validate.Length(equal=20), required=False, allow_none=True, load_only=True
    )
    requested_group_tags = fields.List(
        fields.String(validate=validate.Length(equal=20)), load_only=True, load_default=[]
    )
...
class ResolveGroupRequestSchema(Schema):
    ...
    resolved_group_type = fields.String(
        required=False, load_only=True, validate=validate.OneOf(["okta_group", "role_group", "app_group"])
    )
    ...
    def convert_to_utc(self, item: Dict[str, Any], many: bool, **kwargs: Any) -> Dict[str, Any]:
        # Ensure the datetime we store in the database is UTC
        if (
            "resolved_ownership_ending_at" in item and item["resolved_ownership_ending_at"] is not None
        ):  # Changed from approved_ownership_ending_at
