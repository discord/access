...
class OktaService:
    ...
    async def async_add_user_to_group(self, groupId: str, userId: str) -> None:
        ...
        try:
            async with self._okta_client() as client:
                _, error = await OktaService._retry(client.add_user_to_group, groupId, userId)
    ...
    async def async_remove_user_from_group(self, groupId: str, userId: str) -> None:
        ...
        try:
            async with self._okta_client() as client:
                _, error = await OktaService._retry(client.remove_user_from_group, groupId, userId)
    ...
    def create_group_push_mapping(self, appId: str, sourceGroupId: str, targetGroupId: str) -> dict[str, Any]:
        if not appId:
    ...
    def list_group_push_mappings(self, appId: str) -> list[dict[str, Any]]:
        if not appId:
            raise ValueError("appId is required")
        ...
        async def _list() -> list[dict[str, Any]]:
            async with self._okta_client() as client:
                ...
                while response.has_next():
                    more, error = await OktaService._retry(response.next)
                    if error is not None:
                        raise Exception(error)
