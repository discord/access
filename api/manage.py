...
def _import_from_okta() -> None:
    ...
    if db.session.query(db.func.count(OktaUserGroupMember.id)).scalar() > 0:
        click.echo(
            "Skipping import of Okta Group Memberships as they were previously imported"
        )
    else:
        ...
        for group in groups:
            ...
            for member in members:
                if member.get_deleted_at() is None:
                    db.session.add(
                        OktaUserGroupMember(user_id=member.id, group_id=group.id)
                    )
        db.session.commit()
...
def _init_builtin_apps(admin_okta_user_email: str) -> None:
    ...
    existing_app = (
        App.query.filter(App.name == App.ACCESS_APP_RESERVED_NAME)
        .filter(App.deleted_at.is_(None))
        .first()
    )
    if existing_app is not None:
        click.echo(
            "Access app and groups already exist, skipping init of built-in apps"
        )
        return
    ...
    if admin_okta_user is None:
        click.echo(
            f"Admin Okta user not found with email or id {admin_okta_user_email}"
        )
        return
    ...
    CreateApp(
        ...
        app={'name': App.ACCESS_APP_RESERVED_NAME,
             'description': "The Access Portal"},
    ).execute()
...
@click.option(
    ...
    show_default=True,
    default=False,
    help="If set will notify owners instead of individuals"
)
@with_appcontext
