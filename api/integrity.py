...
def verify_and_fix_unmanaged_groups(dry_run: bool = False) -> None:
    active_unmanaged_groups = OktaGroup.query.filter(
        OktaGroup.is_managed.is_(False)
    ).filter(
...
def verify_and_fix_role_memberships(dry_run: bool = False) -> None:
    ...
    for active_role_group_map in active_role_group_maps:
        ...
        missing_group_users_for_role = set(active_role_group_members_ids) - set(
            active_group_users_for_role_ids
        )
            ...
            if not dry_run:
                for member in list(missing_group_users_for_role):
                    role_group_membership = active_role_group_members_query.filter(
                        OktaUserGroupMember.user_id == member).first()
                    ...
                    if active_role_group_map.ended_at is None:
                        ...
                        associated_users_ended_at = (
                            min(active_role_group_map.ended_at, role_group_membership.ended_at)
                        )
                    db.session.add(
                        OktaUserGroupMember(
                            user_id=member,
        ...
        extra_group_users_for_role = set(active_group_users_for_role_ids) - set(
        ...
        if len(extra_group_users_for_role) > 0:
            ...
            if not dry_run:
                ...
                OktaUserGroupMember.query.filter(
                    ...
                    OktaUserGroupMember.role_group_map_id == active_role_group_map.id
                ).filter(
                ...
                removed_users_with_other_access_ids = [
                    m.user_id for m in removed_users_with_other_access
                ]
                okta_users_to_remove_ids = set(extra_group_users_for_role) - set(
                    removed_users_with_other_access_ids
                )
                for user_id in okta_users_to_remove_ids:
                    if not active_role_group_map.is_owner:
                        ...
                        # https://help.okta.com/en-us/Content/Topics/identity-governance/group-owner.htm
                        okta.remove_owner_from_group(
                            active_role_group_map.group_id, user_id
                        )
