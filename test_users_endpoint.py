...
os.environ["CURRENT_OKTA_USER_EMAIL"] = "wumpus@discordapp.com"
...
with flask_app.app_context():
    ...
    # Test user search
    try:
        response = client.get("/api/v2/users?q=glen")
        print("🔍 GET /api/v2/users?q=glen")
        print(f"   Status: {response.status_code}")
        ...
        if response.status_code == 200:
            ...
            if users:
                ...
                print("   ℹ️ No users found matching 'glen'")
        else:
            print(f"   ✗ Failed: {response.text}")
