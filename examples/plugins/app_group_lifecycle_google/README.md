...
Run the test suite with `tox` from the repository root (its pytest invocation collects the
whole repository, including this plugin, so the plugin suite runs alongside the core Access
tests both locally and in CI):

```bash
tox
```

To run just this plugin's tests, or a single test, pass the path through to pytest via the
`test` environment:
...
# A single test
tox -e test -- examples/plugins/app_group_lifecycle_google/test_plugin.py::test_metadata
```